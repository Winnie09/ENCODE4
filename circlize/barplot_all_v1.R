...
# unique(ad[ad$celltype=='neuromuscular junction nucleus' & ad$species == 'Mus musculus', c('tissue', 'CL')]) ## "CL:0000388"

unique(ad[grep('capsule',ad$celltype), c('celltype', 'species', 'CL','tissue')]) #"CL:0002132" = stromal cell of ovary, Under stromal cell
...
# colnames(d)[colnames(d)== 'celltype'] <- 'celltype.tmp'
# colnames(d)[colnames(d)== 'generaltissue'] <- 'celltype'
...
# barplot(t(t.human), beside = FALSE, col = barcolor, legend = colnames(t.human))
# title("Stacked Bar Plot")
# xlabel <- barplot(t(t.human), plot=FALSE) # Get the midpoints of the bars
# axis(1, at = xlabel, labels = rownames(t.human))
# ylabel <- barplot(t(t.human), plot=FALSE, beside = TRUE) # Get the midpoints of the categories
# axis(2, las=2) # Rotate y-axis labels for readability
