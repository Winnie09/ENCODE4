...
# cut=cutree(hc.human,10) ## from 8 to 6
# dend=color_branches(as.dendrogram(hc.human),k=length(unique(cut)),
#   col=labelcolor[unique(cut[labels])])


## both human an mouse
pdf('/Users/wenpinhou/Dropbox/encode4/circlize/cir_atac_all.pdf',width=10,height=10)
circos.clear()
...
lgd_points = Legend(at = colnames(t.human), type = "points", 
