...
## check celltypes
unique(ad.bk[grep('alveolar', ad.bk$celltype),c('species','celltype','tissue','generaltissue','CL')])
...
## rename the celltypes in ad, using matchtb 
...
## add cell subtype numbers to cell type names, and rename 
...
for (i in 1:length(ctv)){
  ...
  if (length(which(ad.bk[,7] == ctv[i] & ad.bk[,1] == 'Homo sapiens'))>0) len1 <- len1 + 1 # count in original cell types, make sure to use ad.bk
  if (length(which(ad.bk[,7] == ctv[i] & ad.bk[,1] == 'Mus musculus'))>0) len2 <- len2 + 1 # count in original cell types, make sure to use ad.bk
...
## =============================================================================
## use CL terms similarities on the ontology tree to cluster and order celltypes
## generate a table to match unique celltypes and their CL terms
uct = gsub('\\(.*', '',rownames(at))
ucl <- sub(',.*','', ad[match(uct, ad.bk$celltype),'CL'])
cldf = data.frame(uct=uct, ucl=ucl)
# cat(paste0("cldf[cldf[,1]=='",cldf[is.na(cldf[,2]),1],"',2] <- ''"),sep='\n')
cldf[cldf[,1]=='alveolar cell',2] <- 'CL:0002062'
cldf[cldf[,1]=='cortical cell of adrenal gland',2] <- 'CL:0002097'
cldf[cldf[,1]=='neuron',2] <- 'CL:0000540'
cldf[cldf[,1]=='stromal cell',2] <- 'CL:0000499'
cldf

## use ontoProc to construct the ontology tree
library(ontoProc)
library(SummarizedExperiment)
cl = getOnto("cellOnto")
k <- setdiff(unlist(strsplit(cldf[,2],', ')),NA)

pl = ontologyPlot::onto_plot(cl,k)
pdf('/Users/wenpinhou/Dropbox/encode4/circlize/CL_ontoplot.pdf',height=10,width=30)
pl
dev.off()
...
library(igraph)
g <- graph_from_graphnel(gnel)
m <- distances(g)
rownames(m) <- colnames(m) <- gsub('\n',' ',rownames(m))
m <- m[sub('.* ','',rownames(m)) %in% k,sub('.* ','',colnames(m)) %in% k]
rownames(m) <- colnames(m) <- gsub('.*CL','CL',rownames(m))
m2 <- m[match(cldf[,2], rownames(m)), match(cldf[,2], colnames(m))]
m <- m2
rownames(m) <- colnames(m) <- rownames(at)
saveRDS(m, '/Users/wenpinhou/Dropbox/encode4/circlize/CL_dist.rds')

## hclu and dendrogram of the celltypes
hclu <- hclust(as.dist(m))
dend <- as.dendrogram(hclu)
pdf('/Users/wenpinhou/Dropbox/encode4/circlize/CL_dend.pdf',height=30,width=10)
par(mar=c(10,10,10,10) + 5)
plot(dend,horiz = TRUE)
dev.off()

## use hcluster's order as the order of celltypes
overallorder <- gsub(' CL.*', '', rownames(m))[hclu$order]


# ## =======================================================
# ## commented: do not use the cellprop to cluster celltypes
# ## =======================================================
