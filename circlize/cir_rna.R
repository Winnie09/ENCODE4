...
barcolor=colorRampPalette(readRDS('/Users/wenpinhou/Dropbox/resource/color74.rds'))(length(unique(c(colnames(t.human), colnames(t.mouse)))))
...
pdf('/Users/wenpinhou/Dropbox/encode4/data/celltype/proc/cir_rna_human.pdf',width=9,height=9)
circos.clear()
...
## mouse
pdf('/Users/wenpinhou/Dropbox/encode4/circlize/cir_rna_mouse.pdf',width=9,height=9)
circos.clear()
circos.par(cell.padding=c(0,0,0,0))
circos.initialize('a', xlim=c(0,nrow(t.mouse)))
circos.track(ylim=c(0,1),track.height=.2,track.margin=c(0,0),bg.border=NA,
  panel.fun=function(x,y)for(i in 1:nrow(t.mouse))circos.text(i-.5,0,labels[i],adj=c(0,.5),
    facing="clockwise",niceFacing=T,cex=.75))
# col=labelcolor[cut[labels[i]]]

circos.track(ylim=c(0,1),track.height=.1,track.margin=c(0,.01),bg.border=NA,
  panel.fun=function(x,y)circos.barplot(as.matrix(t.mouse)[hc.mouse$order,],-.5+1:nrow(t.mouse),
    col=barcolor,bar_width=1,lwd=.3,border="gray20"))

circos.track(ylim=c(1,0),track.height=.1,track.margin=c(0,.01),bg.border=NA,
  panel.fun=function(x,y)circos.barplot(numCell.mouse[hc.mouse$order],-.5+1:nrow(t3),
    col='grey86',bar_width=1,lwd=.3,border="grey20"))
