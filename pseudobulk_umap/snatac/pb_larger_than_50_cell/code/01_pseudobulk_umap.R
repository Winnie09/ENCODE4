...
pdir <- '/home/whou10/scratch4/whou10/encode4/pseudobulk_umap/snatac/pb_larger_than_50_cell/plot/'
rdir <- '/home/whou10/scratch4/whou10/encode4/pseudobulk_umap/snatac/pb_larger_than_50_cell/result/'
...
pd <- pd[rowMeans(pd > median(pd[pd > 0])) > 0.1, ] ## [1:27612, 1:102] 
str(pd)
dim(pd)
#pd.bak = pd
...
pc <- PCA(genebycellmat = pd, save.pca = TRUE, plot.statistics=FALSE, 
          ...
          maxVariableGenes = 2e3, numPC = NULL, smoothFittingMethod = 'loess') 
## 17 pc selected
