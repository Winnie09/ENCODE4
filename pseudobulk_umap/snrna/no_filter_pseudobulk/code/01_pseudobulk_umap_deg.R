...
pdir <- '/home/whou10/scratch4/whou10/encode4/pseudobulk_umap/snrna/no_filter_pseudobulk/plot/'
rdir <- '/home/whou10/scratch4/whou10/encode4/pseudobulk_umap/snrna/no_filter_pseudobulk/result/'
pd <- readRDS('/home/whou10/scratch4/whou10/encode4/data/pseudobulk/snrna/pb/pb.rds')  
pd <- pd[rowMeans(pd > 1) > 0.1, ] ## [1:26561, 1:350]
str(pd)


## PCA and UMAP
pc <- PCA(genebycellmat = pd, save.pca = TRUE, plot.statistics=FALSE, plot.dir = pdir, result.dir = rdir, PC_for_columns = TRUE, findVariableGenes = TRUE, findVariableGenesMethod = 'lm', maxVariableGenes = NULL, numPC = NULL, smoothFittingMethod = 'loess') ## 13 pc selected
