...
pdir <- rdir <- '/home/whou10/scratch4/whou10/encode4/pseudobulk_umap/snrna/pb_ambient/parseshallow/'
# pd <- readRDS(paste0(ddir, '10x.rds'))
pd <- readRDS(paste0(ddir, 'parseshallow.rds'))
str(pd)
colnames(pd)[1]
...
for (id in 1:length(myvar)){
  ...
  pdf(paste0(pdir, 'pca_ggrepel_by_', var, '.pdf'), width = 2.5, height = 2.5)
