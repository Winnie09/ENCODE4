...
rm(list=ls())
## --------------------------------------
## snRNA pseudobuk: remove ambient
## --------------------------------------
## read in pseudobulk
source('/home/whou10/scratch16/whou10/resource/myfunc/01_function.R')
ddir <- '/home/whou10/scratch4/whou10/encode4/data/pseudobulk/snrna/ambient/'
pdir <- rdir <- '/home/whou10/scratch4/whou10/encode4/pseudobulk_umap/snrna/pb_ambient/10x/'
# rdir2 <- '/home/whou10/scratch4/whou10/encode4/pseudobulk_umap/snrna/pb_ambient/parseshallow/'
pd <- readRDS(paste0(ddir, '10x.rds'))
# d2 <- paste0(ddir, 'parseshallow.rds')
str(pd)
colnames(pd)[1]
...
pc <- PCA(genebycellmat = pd, save.pca = TRUE, plot.statistics=FALSE, 
          plot.dir = rdir, result.dir = rdir, PC_for_columns = TRUE, 
...
u <-
  data.frame(
    ...
    celltype = as.vector(sapply(sub('.*:', '', rownames(u)), tools::toTitleCase))
  )
str(u)
length(unique(u[,3]))
length(unique(u[,4]))
...
for (id in 1:length(myvar)){
  ...
  pdf(paste0(pdir, 'pca_ggrepel_by_', var, '.pdf'), width = ifelse(id ==1, 2.5, 2.5), height = ifelse(id ==1, 2.5, 2.5))
...
pdf(paste0(pdir, 'pca_facet_tissue.pdf'), width = 5.5, height = 4.5)
ggplot(data = u, aes_string(x = colnames(u)[1], y = colnames(u)[2], color = colvar)) +
  geom_point( size = 0.2) + 
  scale_color_manual(values = colorRampPalette(rev(brewer.pal(8, 'Set1')))(length(unique(u[, colvar])))) + 
  facet_wrap(~tissue) + 
