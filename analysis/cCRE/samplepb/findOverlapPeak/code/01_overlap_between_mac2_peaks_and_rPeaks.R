...
pdf(paste0(pdir, 'numPeakOverlap_human.pdf'), width = 16, height = 6)
ggplot(df, aes(x = Tissue, y = numPeakOverlap, color = Tissue, alpha = 0.2, fill = Tissue)) + 
...
# numPeakOverlap vs. tissue
pdf(paste0(pdir, 'numPeakOverlap_mouse.pdf'), width = 2.5, height = 4.5)
ggplot(df, aes(x = Tissue, y = numPeakOverlap, color = Tissue, alpha = 0.2, fill = Tissue)) + 
  geom_violin(aes(alpha = 0.1), width=1, scale = 'width') +
  geom_boxplot(aes(alpha = 0.3), width=0.4, color="black", alpha=0.6) +
  geom_jitter(color = 'black', alpha = 0.5, size = 0.8, width = 0.3)+
