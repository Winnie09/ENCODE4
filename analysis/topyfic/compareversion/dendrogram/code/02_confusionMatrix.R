...
numcell <- sapply(alltissue, function(tissue){
  ddir1 <- paste0(ddir, tissue, '/figures/')
  if (file.exists(paste0(ddir1, 'cell_participation.csv'))){
    tb = read.csv(paste0(ddir1, 'cell_participation.csv'), row.names = 1)
    nrow(tb)
  } else {
    NA
  }
...
for (tissue in sort(names(numcell))){
  tryCatch({
    ...
    print(getwd())
    dir.create(paste0(getwd(), '/', pdir, 'hclu/'))
    pdf(paste0(getwd(), '/', pdir, 'hclu/', tissue,'.pdf'))
