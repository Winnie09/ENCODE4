...
library(ggplot2)
library(gridExtra)
pdf(paste0(pdir, 'pcc_by_sd_acrossgene.pdf'), width = 2.2, height = 4.2)
...
pdf(paste0(pdir, 'pcc_by_experiment_acrossgene.pdf'), width = 8.3, height = 2)
ggplot()+
...
pd[,11] <- as.character(pd[,11])
pd[,12] <- as.character(pd[,12])

assign('var5', 'tissue')
pdf(paste0(pdir, 'pcc_by_tissue_acrossgene.pdf'), width = 9, height = 2.5)
ggplot()+
...
assign('var5', 'species')
pdf(paste0(pdir, 'pcc_by_species_acrossgene.pdf'), width = 9, height = 2.5)
ggplot()+
  #geom_jitter(data = pd[sample(1:nrow(pd), 1e5),], aes_string(x = var5, y = var2, color = var4), 
  #            size = 0.2, stroke = 0, alpha = 0.8, width = 0.15) + 
  theme(axis.text.x = element_text(angle = 45,vjust = 0.6))  +
