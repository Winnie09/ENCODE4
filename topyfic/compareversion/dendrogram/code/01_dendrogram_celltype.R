...
alltissue = list.files(commandArgs(trailingOnly = T)[1])
ddir1 = commandArgs(trailingOnly = T)[2]
pdir = commandArgs(trailingOnly = T)[3]
rdir = commandArgs(trailingOnly = T)[4]
...
for (tissue in alltissue){
  ...
  png(paste0(pdir, tissue, '.png'), res = 300, width = 3500, height = 3000)
  print(p)
  dev.off()
}
